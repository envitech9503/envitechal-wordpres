...
The public assistant is provider-independent by default. Its health route validates the built-in, source-linked Envi Tech AL answer catalogue and never waits for a model or knowledge base. Verified catalogue questions are answered immediately; unmatched questions receive a canonical contact-and-services response instead of a speculative answer or a slow provider error. DigitalOcean generation is an optional enhancement and remains disabled unless `ETA_AGENT_REMOTE_ENABLED` is deliberately set to `true`, `yes`, `on`, or `1` in a server-side constant, environment value, or WordPress option after grounded retrieval has passed an operator test. Never enable that flag merely because `/health` or `/readyz` returns 200: a real authorized completion with retrieval and citations must pass first.
